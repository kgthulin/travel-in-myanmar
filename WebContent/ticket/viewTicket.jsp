		...
		<div class="row">
			<div class="col-md-12">
				<div style="margin-bottom: 10px; text-align: right;">
					<s:url id="loc" action="setUpForTicketLocation" />
					<s:a href="%{loc}" cssClass="btn btn-info btn-md" tabindex="-1"
						role="button" aria-disabled="true">Create New Ticket</s:a>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-md-2">
				<div style="background-color: #5eb2b3">
					<ul class="nav flex-column">
						<li class="nav-item"><s:url id="viewPac"
								action="ViewPackageAction" /> <s:a cssClass="nav-link"
								href="%{viewPac}">Package</s:a></li>
						...
						<li class="nav-item"><s:url id="viewTicket"
								action="ViewTicketAction" /> <s:a cssClass="nav-link active"
			...
			<div class="col-md-10">
				<table class="table table-hover" style="background: #adebeb">
					<thead>
						<tr>
							<th scope="col">No</th>
							<th scope="col">Name of Line</th>
							<th scope="col">Price</th>
							<th scope="col">Location</th>
						</tr>
					</thead>
					<tbody>
						<s:iterator value="ticketList" status="status">
							<tr>
								<th scope="row"><s:property value="#status.count" /></th>
								<td><s:property value="line_name" /></td>
								<td><s:property value="price" /></td>
								<td><s:property value="location.name" /></td>
								<td><s:url id="update" action="setUpForTicketUpdate">
										<s:param name="currentId" value="ticket_id" />
									</s:url> <s:a href="%{update}" cssClass="btn btn-info btn-sm"
										role="button" aria-disabled="true">Edit</s:a></td>
								<td><s:url id="delete" action="DeleteTicketAction">
										<s:param name="currentId" value="ticket_id" />
